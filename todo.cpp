...
void TodoList::add_task(const char* task) {
    if (size >= MAX_TASKS) throw std::overflow_error("Full tasks");
...
void TodoList::remove_task(int index) {
    if (index < 0 || index >= size) throw std::out_of_range("Wrong index");

    delete[] tasks[index];
    for (int i = index; i < size - 1; i++) tasks[i] = tasks[i + 1];
